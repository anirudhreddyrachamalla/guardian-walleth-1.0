...
contract MultiSigWallet {
    ...
    constructor(
        ...
        if (_numOfConfirmationsRequired > 0) {
            bool isNumberOfConfirmationsRequiredValid = _numOfConfirmationsRequired >
                    0 &&
                    _numOfConfirmationsRequired <= _approvers.length;
            require(
                isNumberOfConfirmationsRequiredValid,
                "Please enter valid number of confirmations required for a transaction"
            );
    ...
    modifier onlyMultiSig() {
        require(
            numOfConfirmationsRequired > 0,
            "Allowed for multisig wallets only"
        );
        _;
    }
    ...
    function initiateTransaction(
        ...
        require(
            isTransactionInitiatedByOwner,
            "Only owner can initaite a transaction"
        );
        ...
        returns (uint)
    {
        ...
        bool hasTransactionInitiated = transactions[_txIndex].initiationTime >
            0;
    ...
    function getStatusOfYourApproval(uint _txIndex)
        external
        view
        returns (bool)
    {
        ...
        returns (uint)
    {
        ...
        require(
            hasAlreadyApproved,
            "You have NOT approved the transaction YET"
        );
    ...
    function publishTransaction(uint _txIndex) external {
        ...
        (bool sent, ) = transactions[_txIndex].to.call{
            value: transactions[_txIndex].amount
        }(transactions[_txIndex].data);
        require(sent, "Failed to send Ether");
        transactions[_txIndex].executed = true;
    }

    function getNumberOfConfirmations() external view returns (uint) {
    ...
    function getTransaction(uint _txIndex)
        public
        view
        returns (address to, uint amount)
    {
