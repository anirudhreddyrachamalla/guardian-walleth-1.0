...
contract SmartWallet is AutomationCompatibleInterface {
    ...
    function initiateTransaction(
        ...
        uint txIndex = wallets[msg.sender].multiSigWallet.initiateTransaction(
            _to,
            _amount,
            _data
        );
        uint numConfirmationsRequired = wallets[msg.sender]
            .multiSigWallet
        ...
        address[] memory approvers = wallets[msg.sender]
            .multiSigWallet
            .fetchApproverData();
    ...
    function getNumberOfConfirmationsDone(uint _txIndex)
        external
        view
        returns (uint)
    {
        uint result = wallets[msg.sender]
            .multiSigWallet
            .getNumberOfConfirmationsDone(_txIndex);
        return result;
    }

    function approveTransaction(uint _txIndex, address _owner) external {
        uint numConfirmationsDone = wallets[_owner]
            .multiSigWallet
            .approveTransaction(_txIndex);
        uint numConfirmationsRequired = wallets[_owner]
            .multiSigWallet
            .getNumberOfConfirmations();
        ...
        emit TransactionStatus(
            _owner,
            _txIndex,
            numConfirmationsDone,
            numConfirmationsRequired
        );
    }

    function getApprovalStatus(uint _txIndex) external view returns (bool) {
        bool result = wallets[msg.sender]
            .multiSigWallet
            .getStatusOfYourApproval(_txIndex);
        return result;
    }
    ...
    // SocialRecovery
    function castRecoveryVote(address walletOwner, address _newOwnerAddress)
        public
    {
    ...
    function removeElementFromArray(address guardian, address element)
        internal
    {
        uint i;
    ...
    function fetchGuardianData() public view returns (GuardianUIData[] memory) {
        address[] memory existingGuardians = wallets[msg.sender]
            .socialRecovery
            .fetchExistingList();
        GuardianUIData[3] memory result;
        for (uint i; i < existingGuardians.length; i++) {
            result[i] = GuardianUIData(
                existingGuardians[i],
                wallets[msg.sender].socialRecovery.fetchGuardianStatus(
                    existingGuardians[i]
                )
            );
        }
    ...
    {
        (address to, uint amount) = wallets[msg.sender]
            .multiSigWallet
